...
Set.prototype._migrate = function (direction, fn, lastMigrationNum) {
	...
	function next(err, migration) {
		...
		const num = migration.num || parseInt(migration.title.match(/\d+/)[0].split('-')[0], 10);
		const lockCol = self.db.collection('migration_lock');
		lockCol.find({num: num}).limit(1).toArray(function(err, migrationsRun) {
			if (err) {
				console.error('Error inside check migration lock', err);
				process.exit(1);
			}

			if (migrationsRun && migrationsRun.length > 0) {
				console.error('Migration Locked');
				process.exit(1);
			}
			...
			lockCol.insert({
				...
				if (err) {
					console.error('Error inside insert migration lock', err);
					process.exit(1);
				}
