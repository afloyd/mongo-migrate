...
function getMigrationsCollection(db, mongodb, trackingCollection) {
	// for mongodb 2.x
	if (typeof db.collection !== 'undefined') {
		return db.collection(trackingCollection);
	}

	return new mongodb.Collection(db, trackingCollection);
}

function getConnection(trackingCollection, opts, cb) {
	...
	if (opts.connectionString) {
		...
		MongoClient.connect(opts.connectionString, function(err, db) {
			...
			cb(null, {
				connection: db,
				migrationCollection: getMigrationsCollection(db, mongodb, trackingCollection)
			});
		});
		return;
	}
